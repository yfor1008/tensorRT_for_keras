...
if __name__ == '__main__':
    ...
    for data_dir in data_dirs:
        ...
        for batch_idx in range(batch_num):
            ...
            prediction = np.argmax(engine_fp32.infer(imgs)[0], 1).tolist()
            pre = [ff[0][0] for ff in prediction]
            predictions.extend(pre)

        end_time = time.time()
        with open(os.path.join('./results/', 'time_for_fp32.txt'), 'a') as ff:
            ff.write('%s: %f \n' % (os.path.basename(data_dir), end_time - start_time))
        scio.savemat(os.path.join('./results/results_for_fp32', os.path.basename(data_dir) + '.mat'), {'data': predictions})
